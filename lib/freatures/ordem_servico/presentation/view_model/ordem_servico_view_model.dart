...
OrderStatus _parseStatus(String? value) {
  switch (value) {
    case 'completed': return OrderStatus.completo;
    case 'completo': return OrderStatus.completo;
    case 'encaminhada': return OrderStatus.encaminhada;
    case 'inProgress': return OrderStatus.encaminhada;
    case 'pedentes': return OrderStatus.pedentes;
    case 'paymentPending': return OrderStatus.pedentes;
    case 'cancelados': return OrderStatus.cancelados;
    case 'cancelled': return OrderStatus.cancelados;
    case 'open': return OrderStatus.aberto;
    case 'aberto': return OrderStatus.aberto;
    default: return OrderStatus.aberto;
  }
}

Future<void> carregarOrdensPorFuncionario(String funcionarioId, {bool isAdmin = false}) async {
    try {
      _ordens.clear();
      notifyListeners();
      await _ensureFirebaseAuth();
    ...
    try {
      ...
      try {
        await FirebaseFirestore.instance
          ...
          'equipamento': osCompletaComValores.equipamento,
          'tipoDefeito': osCompletaComValores.tipoDefeito,
          'modeloEquipamento': osCompletaComValores.modeloEquipamento,
          'metragemAmbiente': osCompletaComValores.metragemAmbiente,
          'tensaoEletrica': osCompletaComValores.tensaoEletrica,
          'equipamentoAvaliado': osCompletaComValores.equipamentoAvaliado,
          'diagnostico': osCompletaComValores.diagnostico,
          'solucaoRecomendada': osCompletaComValores.solucaoRecomendada,
        });
      } catch (e) {
        print('Erro ao salvar OS no Firestore: $e');
        throw Exception('Erro ao salvar OS no Firestore: $e');
      }
      try {
